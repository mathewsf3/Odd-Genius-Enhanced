...
export class CornersAnalysisService {
  private readonly CORNERS_THRESHOLDS = [6.5, 7.5, 8.5, 9.5, 10.5, 11.5, 12.5, 13.5];

  /**
   * Analyze corners data for a team's matches
   */
  analyzeCorners(matches: HistoricalMatch[], isHomeTeam: boolean = true): CornersAnalysis {
  ...
  private calculateCornersThresholds(matches: HistoricalMatch[]): ThresholdStatistics[] {
    return this.CORNERS_THRESHOLDS.map(threshold => {
      const matchesOver = matches.filter(match => 
        match.statistics.corners.total > threshold
      ).length;
  ...
  private calculateAverageCorners(matches: HistoricalMatch[]): number {
  ...
  private calculateAverageCornersFor(matches: HistoricalMatch[], isHomeTeam: boolean): number {
    if (matches.length === 0) return 0;
  ...
  private calculateAverageCornersAgainst(matches: HistoricalMatch[], isHomeTeam: boolean): number {
    if (matches.length === 0) return 0;
  ...
  private calculateFirstHalfAverage(matches: HistoricalMatch[]): number {
    const firstHalfMatches = matches.filter(match => match.statistics.corners.firstHalf);
    if (firstHalfMatches.length === 0) return 0;
    
    const totalFirstHalfCorners = firstHalfMatches.reduce((sum, match) => 
      sum + (match.statistics.corners.firstHalf?.total || 0), 0
    );

    return parseFloat((totalFirstHalfCorners / firstHalfMatches.length).toFixed(2));
  }

  private calculateSecondHalfAverage(matches: HistoricalMatch[]): number {
    const secondHalfMatches = matches.filter(match => match.statistics.corners.secondHalf);
    if (secondHalfMatches.length === 0) return 0;
    
    const totalSecondHalfCorners = secondHalfMatches.reduce((sum, match) => 
      sum + (match.statistics.corners.secondHalf?.total || 0), 0
    );
  ...
  /**
   * Get corners statistics for specific thresholds
   */
  getCornersOverUnderStats(matches: HistoricalMatch[], thresholds: number[] = this.CORNERS_THRESHOLDS): Map<number, ThresholdStatistics> {
  ...
  /**
   * Calculate team-specific corners statistics
   */
  calculateTeamCornersStats(matches: HistoricalMatch[], isHomeTeam: boolean): {
    ...
    cornersEfficiency: {
      highCornerGames: number; // games with 6+ corners won
      lowCornerGames: number; // games with 0-2 corners won
    };
  } {
    if (matches.length === 0) {
      return {
    ...
    const getDominance = (won: number, conceeded: number): 'dominant' | 'balanced' | 'weak' => {
      const ratio = won / (conceeded || 1);
      if (ratio >= 1.5) return 'dominant';
      if (ratio <= 0.67) return 'weak';
      return 'balanced';
    };
  ...
  /**
   * Analyze corners distribution by half
   */
  analyzeCornersDistributionByHalf(matches: HistoricalMatch[]): {
    ...
    const matchesWithHalfData = matches.filter(match => 
      match.statistics.corners.firstHalf && match.statistics.corners.secondHalf
    );
    ...
    const getPattern = (fhPerc: number, shPerc: number): 'first_half_heavy' | 'second_half_heavy' | 'balanced' => {
      if (fhPerc > 60) return 'first_half_heavy';
      if (shPerc > 60) return 'second_half_heavy';
      return 'balanced';
    };
  ...
  /**
   * Analyze home/away corners advantage
   */
  ...
  /**
   * Calculate corners-to-goals conversion rate
   */
...
export default new CornersAnalysisService();
